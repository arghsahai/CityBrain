...
Krishna owns `simulation/` and `experiments/`. The experiment runner calls the existing CityState, state adapter, EmergencyPlanner and Replanner. It does not replace teammate agents or routing architecture. Remote teammate branches differ substantially and have not been merged. Remote `develop` was absent during inspection.
...
Detect stopped blockers and traffic → update CityState → build live planner state → evaluate reachable candidate route suffixes → call the existing Replanner → apply through TraCI → measure → repeat.
...
Legacy controllers keep their old CSV columns and append `completion_status` and `teleported`. Existing checked-in legacy CSVs predate the correction and must not be used as valid evidence. New experiments use the separate paired CSV schema; readers should select fields by name. Do not combine old and new schemas.
