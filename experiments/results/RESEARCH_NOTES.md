...
Post-validation formatting: the final diff audit removed trailing indentation whitespace from the research network's node/edge source XML and made its builder emit clean text. The compiled .net.xml and experiment execution code did not change. validation_manifest.json records before/after hashes. Historical per-run manifests remain untouched; strict resume from this final tree correctly rejects their changed source bytes. Use the validated df36103 execution snapshot in a separate checkout for those exact manifests, or choose fresh output for future changed inputs. All 120 trials are already complete, so no reruns were needed.
